...
export const createTaskTool = {
    ...
    description: "Create a new ListenME product research task. Returns the created task ID. " +
    ...
    inputSchema: {
        ...
        parameters: z.record(z.unknown()).describe("Task parameters object containing: domain (string), productUrl (string), " +
        ...
        rewardInfo: z.record(z.unknown()).optional().describe("Reward configuration: chainId, tokenAddress, tokenSymbol, amount, limit, currency"),
    ...
    handler: async (args) => {
        ...
        const res = await walletRequest("POST", path, {
            body: body,
            signParams,
        });
        if (res.code < 0) {
        ...
        return { content: [{ type: "text", text: `Task created successfully. Task ID: ${res.data}` }] };
    },
};
