...
import type { FeedbackTopicItem } from "../types/feedback.js";

export const getTopicsTool = {
  name: "listenme_get_topics",
  description:
    "Get extracted topics from feedback for a ListenME task. Returns topic names, " +
  ...
  handler: async (args: { taskId: string; startTime?: number; endTime?: number }) => {
    ...
    const res = await aiRequest<FeedbackTopicItem[]>("GET", path, {
      params,
      signParams: { ...params },
    });
